class Kursy
	...
	def dodajKurs(waluta, sprzedaz, kupno, wzrost)
		if((sprzedaz.to_f > 0) && (kupno.to_f > 0))
			...
			puts "\n"
			puts '!!Nie mozna dodac waluty o niedodatnim kursie!!'
			puts "\n"
		end
	end

	#SPRAWDZ POPRAWNOSC WALUTY
	def sprawdzWaluteKurs(waluta)
		...
		puts "\n"
		puts "!!Operacja zostaje przerwana, taka waluta nie istnieje!!"
		puts "\n"
		return 0
	end
	...
	def aktualizujKursy()
		for i in 0..(@arr.length-1)
			modyfikator = (((@arr[i][3].to_f)*(-1))/4)
			x = rand(0..500)
			@arr[i][3] = (((x - 250.0)/5000.0) + modyfikator).to_s
			@arr[i][1] = (@arr[i][1].to_f + (@arr[i][1].to_f * @arr[i][3].to_f)).to_s
			@arr[i][2] = (@arr[i][2].to_f + (@arr[i][1].to_f * @arr[i][3].to_f)).to_s
		end
	end
	...
	def pokazKursy()
		...
		puts "\n\n"
		puts '(wcisnij ENTER, aby wyjsc z kursow walut)'
		gets.chomp
	end
end
...
class Portfel
	...
	def pobierzZPortfela(waluta, ilosc)
		...
		puts "\n"
		puts '!!Operacja nie zostala wykonana, taka waluta nie istnieje!!'
		puts "\n"
		return 0
	end

	#SPRAWDZ POPRAWNOSC WALUTY
	def sprawdzWalutePortfel(waluta)
		for i in 0..(@tab.length-1)
			if waluta == (@tab[i][0])
				return 1
			end
		...
		puts "\n"
		puts "!!Operacja zostaje przerwana, nie masz takiej waluty!!"
		puts "\n"
		return 0
	end
	...
	def zwrocIloscWaluty(waluta)
		for i in 0..(@tab.length-1)
			if waluta == (@tab[i][0])
				return (@tab[i][1].to_f).to_s
			end
		end
		puts "\n"
		puts '!!Operacja nie zostala wykonana, wcale nie posiadasz takiej waluty!!'
		puts "\n"
		return 0
	end
	...
	def obliczKapital(kursy)
		kapitalCalkowity = 0
		for i in 0..(@tab.length-1)
			kapitalCalkowity = ((kapitalCalkowity.to_f) + (@tab[i][1].to_f)*(kursy.zwrocKupno(@tab[i][0]).to_f)).to_s
		end
...
def StanKonta(kursy, portfel)
	...
	puts "\n\n"
	puts '(wcisnij ENTER, aby wyjsc z banku)'
	gets.chomp
end
...
def Kantor(kursy, portfel)
	...
	while 1 do
		...
		case r
			...
			if ((kursy.sprawdzWaluteKurs(walutaKupowana)).to_f) == 0
				Kantor(kursy, portfel)
				break
			end
			print '>>Podaj jej ilosc: '
			iloscKupowana = gets.chomp
			if (iloscKupowana.to_f) <= 0
				puts "\n"
				puts "!!Operacja zostaje przerwana, podano niedodatnia ilosc waluty!!"
				puts "\n"
				Kantor(kursy, portfel)
				break
			end
			...
			if ((portfel.sprawdzWalutePortfel(walutaSprzedawana)).to_f) == 0
				Kantor(kursy, portfel)
				break
			end
...
def Menu()
	...
	#WYBIERANIE POCZATKOWEJ KWOTY W PLN
	while 1 do
		puts "\n"
		puts '======================================================================================================'
		...
		if (kapitalStartowy.to_f) <= 0
			puts "\n"
			puts '!!Musisz rozpoczac sesje z dodatnim kapitalem!!'
		else
